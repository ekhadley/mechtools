...
def tabbed(panes: dict[str, str] | dict[str, dict[str, str]], head: str = "", bars: bool = True) -> str:
    ...
    The panes are stacked in one grid cell, so the widget keeps the height of its tallest pane and does not resize as tabs change. Tab labels are escaped; panes are raw html."""
    if not panes:
    ...
    tab_bars = "".join(f"<div class='tb'{' hidden' if len(ls) == 1 or not bars else ''}>{''.join(f'<button>{html.escape(str(l))}</button>' for l in ls)}</div>" for ls in [list(rows), inner])
...
def show_logits(input_src, model=None, logits=None, tokenizer=None, k: int = 10, pos: list[int] | None = None, title: str | None = "logits", ctx: int = 32, n_cols: int = 4):
    ...
    input_src (see get_toks) is a string, ids, a conversation, or str tokens (which cannot be run through a model: pass logits with them); `pos` restricts the readout to those positions, all of them by default."""
...
def cluster_tables(scores: Tensor, labels: Tensor, name, n_clusters: int, n_rows: int, softmax: bool) -> tuple[list, list[int]]:
    """Tables for one cluster readout: the overall top-k with each item's cluster id, then one per top cluster (of the non-empty ones, at most n_clusters) with each item's overall rank. n_rows is clipped to the items available. Returns (tables, shown cluster ids)."""
