...
The tokenizer tests load Qwen3 (0.6B, 8B, 3.6-27B, 3.8-27B), Qwen2.5-3B-Instruct, gemma-3 (1b, 4b), Llama-3 (3.2-1B, 3.1-8B) and Starling-LM-7B-alpha tokenizers from the local HF cache. The `openrouter`, `resample` and `sampling` tests run on fakes and make no API calls (the sampling fake is a scripted model over a real `DynamicCache`, so `sample_rolling`'s cache surgery runs for real); the live checks are `probe` and `sampling_defaults` in `mechtools.resample`, which spend a few cents.
...
| `bars` | `pbar`: tqdm with the house style (colored desc, `ncols=120`, ascii fill). Its kwargs go to tqdm; the `quiet` flags on the samplers and `kmeans` are its `disable` |
...
| `sampling` | `stream_toks`, `stream_toks_hf`, `sample_batch`, `sample_rolling` for a `TransformerBridge`. All stop at any id in `eos_ids`: the HF `generation_config.eos_token_id` (an int or a list) plus the tokenizer's eos, since a chat model ends its turn with a token that is not the tokenizer's eos (gemma-3-it's `<end_of_turn>`, Qwen3's `<|endoftext|>` beside `<|im_end|>`); it raises when neither is set. The batch samplers take `quiet=True` to hide the progress bar |
...
| `stats` | `normed`, `cosine_sim`, `pearson`, `mean_self_sim`, `topk_vector_matches`, `kmeans` (`quiet=True` hides its bar), `hierarchical_kmeans` (k-means then cosine agglomeration of the centroids), `wilson` |
